...
class FileDialog(object):
    ...
    def getSaveFileName(parent=None, title="Save File", file_dir=None, ext_filter="*"):
        ...
        file_dlg = QtWidgets.QFileDialog.getSaveFileName(parent, title, file_dir, ext_filter)
    ...
    def getOpenFileName(parent=None, title="Open File", file_dir=None, ext_filter="*"):
        if not file_dir:
            file_dir = _current_user_directory
        file_dlg = QtWidgets.QFileDialog.getOpenFileName(parent, title, file_dir, ext_filter)
        file = file_dlg[0] or None
        if file:
            _set_dir(file)
        return file_dlg
...
class BaseDialog(object):
    @staticmethod
    def message_dialog(parent=None, text="", title="Message", dialog_icon=None, custom_icon=None):
        ...
        if custom_icon:
            pixmap = QtGui.QPixmap(custom_icon).scaledToHeight(32, QtCore.Qt.TransformationMode.SmoothTransformation)
    ...
    @staticmethod
    def question_dialog(parent=None, text="", title="Are you sure?", dialog_icon=None, custom_icon=None):
        dlg = QtWidgets.QMessageBox(parent=parent)
        dlg.setWindowTitle(title)
        dlg.setInformativeText(text)
