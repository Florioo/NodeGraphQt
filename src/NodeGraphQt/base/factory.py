    ...
    def register_node(self, node, alias=None):
        ...
        if self.__nodes.get(node_type):
            raise NodeRegistrationError(
                ...
                "Please specify a new plugin class name or __identifier__.".format(node_type, self.__nodes[node_type])
            )
        ...
        if alias:
            if self.__aliases.get(alias):
                raise NodeRegistrationError(
                    'Alias: "{}" already registered to "{}"'.format(alias, self.__aliases.get(alias))
                )
