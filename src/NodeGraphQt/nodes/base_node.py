...
from NodeGraphQt.errors import NodeWidgetError, PortError, PortRegistrationError
from NodeGraphQt.qgraphics.node_base import NodeItem
from NodeGraphQt.widgets.node_widgets import NodeBaseWidget, NodeCheckBox, NodeComboBox, NodeLineEdit
...
class BaseNode(NodeObject):
    ...
    def add_custom_widget(self, widget, widget_type=None, tab=None):
        ...
        self.create_property(widget.get_name(), widget.get_value(), widget_type=widget_type, tab=tab)
    ...
    def add_text_input(self, name, label="", text="", placeholder_text="", tooltip=None, tab=None):
        """
        Creates a custom property with the :meth:`NodeObject.create_property`
        ...
        """
        self.create_property(
            name, value=text, widget_type=NodePropWidgetEnum.QLINE_EDIT.value, widget_tooltip=tooltip, tab=tab
        )
    ...
    def add_checkbox(self, name, label="", text="", state=False, tooltip=None, tab=None):
        ...
        """
        self.create_property(
            name, value=state, widget_type=NodePropWidgetEnum.QCHECK_BOX.value, widget_tooltip=tooltip, tab=tab
        )
    ...
    def add_input(
        ...
        if name in self.inputs().keys():
            raise PortRegistrationError('port name "{}" already registered.'.format(name))
    ...
    def delete_input(self, port):
        ...
        if not self.port_deletion_allowed():
            raise PortError(
                'Port "{}" can\'t be deleted on this node because ' '"ports_removable" is not enabled.'.format(
                    port.name()
                )
            )
        if port.locked():
    ...
    def set_ports(self, port_data):
        ...
        if not self.port_deletion_allowed():
            raise PortError(
                "Ports cannot be set on this node because " '"set_port_deletion_allowed" is not enabled on this node.'
            )
